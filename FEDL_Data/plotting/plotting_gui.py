...
from matlab_generator import GroupDefinition, PlotDefinition, SeriesRef, build_matlab_script, default_output_formats, detect_demo_groups
...
5. MATLAB 스크립트 생성 / 실행
- [스크립트 생성]은 generated_plotting_demo.m 파일을 출력합니다.
- [MATLAB 실행]은 R2025a를 호출해 .fig / .png / .svg를 저장합니다.
...
"""
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("FEDL MATLAB Plotting Demo")
    ...
    def _build_ui(self):
        ...
        footer.pack(fill="x")
        watermark = ttk.Frame(footer)
        watermark.pack(side="left")
        ttk.Label(watermark, text="ASNL_HYUNHO.JANG", foreground="#666666").pack(anchor="w")
        ttk.Label(watermark, text="무단 배포 금지", foreground="#666666").pack(anchor="w")
        ttk.Label(watermark, text="문의: hyunho0214@naver.com", foreground="#666666").pack(anchor="w")
        ttk.Button(watermark, text="how to use?", command=self.show_manual).pack(anchor="w", pady=(4, 0))
