...
def select_log_spaced_sigmas_with_geomean_single(
    sigma_data: np.ndarray,
    n_sigmas: int,
) -> np.ndarray:
    """RBF2-compatible sigma selection with geometric mean for N=1."""
    if n_sigmas <= 0:
        raise ValueError("n_sigmas must be positive.")
    sigma_min, sigma_max = get_sigma_range(sigma_data)
    if n_sigmas == 1:
        return np.asarray([np.sqrt(sigma_min * sigma_max)], dtype=float)
    return np.geomspace(sigma_min, sigma_max, n_sigmas)
