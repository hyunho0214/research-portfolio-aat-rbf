...
# 2. CSV 파일에서 초기 실험 데이터 읽어오기
print("hello.csv 파일에서 실험 데이터를 읽어옵니다.")
try:
    csv_data = pd.read_csv('hello.csv')
    print("CSV 데이터:")
    print(csv_data.head())
    ...
    for _, row in csv_data.iterrows():
        # ar과 o2 값을 읽어서 o2/ar 비율 계산
        ar_value = row['ar']
        o2_value = row['o2']
        o2_ar_ratio = o2_value / ar_value if ar_value != 0 else 0  # o2/ar 비율
        thick = row['thickness']
        mobility = row['mobility']

        init_x.append([o2_ar_ratio, thick])
        init_y.append(mobility)
        print(f"데이터: Ar={ar_value}, O2={o2_value}, O2/Ar={o2_ar_ratio:.3f}, 두께={thick}, Mobility={mobility}")
    ...
    print("hello.csv 파일을 찾을 수 없습니다. 수동으로 초기 데이터를 입력합니다.")
    # 기존 수동 입력 코드 사용
    init_x = []
    init_y = []
...
ax2.set_xticklabels([f"{ratio:.1f}" for ratio in ratio_candidates])
ax2.set_yticks(range(len(thickness)))
ax2.set_yticklabels([f"{thick}" for thick in thickness])
...
ax3.set_xticklabels([f"{ratio:.1f}" for ratio in ratio_candidates])
ax3.set_yticks(range(len(thickness)))
ax3.set_yticklabels([f"{thick}" for thick in thickness])
...
print("분석 완료! 전체 데이터셋에 대한 학습 및 예측이 완료되었습니다.")