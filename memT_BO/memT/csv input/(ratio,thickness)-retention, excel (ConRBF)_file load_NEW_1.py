...
def print_hyperparameters_composite(gpr, iteration):
    """
    ConstantKernel * RBF + WhiteKernel 구조의 GPR 하이퍼파라미터 출력

    Parameters:
    -----------
    gpr : GaussianProcessRegressor
        학습된 GPR 모델 (ConstantKernel * RBF + WhiteKernel 구조)
    iteration : int
        현재 반복 횟수
    """
    print(f"\n=== 반복 {iteration + 1}: GPR 하이퍼파라미터 (Composite Kernel) ===")

    # 1. 전체 커널 구조 출력
    print(f"전체 커널 구조: {gpr.kernel_}")

    try:
        # 2. ConstantKernel * RBF + WhiteKernel 구조 분석
        # gpr.kernel_은 Sum 커널 (k1: ConstantKernel*RBF, k2: WhiteKernel)
        composite_kernel = gpr.kernel_

        # Sum의 첫 번째 부분: ConstantKernel * RBF
        if hasattr(composite_kernel, 'k1'):
            product_kernel = composite_kernel.k1  # ConstantKernel * RBF

            # ConstantKernel 파라미터 추출
            if hasattr(product_kernel, 'k1'):
                constant_kernel = product_kernel.k1
                if hasattr(constant_kernel, 'constant_value'):
                    constant_value = constant_kernel.constant_value
                    print(f"ConstantKernel:")
                    print(f"  Constant Value: {constant_value:.6f}")

                    if hasattr(constant_kernel, 'constant_value_bounds'):
                        const_bounds = constant_kernel.constant_value_bounds
                        if const_bounds != 'fixed':
                            print(f"  Constant Value Bounds: ({const_bounds[0]:.2f}, {const_bounds[1]:.2f})")
                        else:
                            print(f"  Constant Value Bounds: fixed")

            # RBF Kernel 파라미터 추출
            if hasattr(product_kernel, 'k2'):
                rbf_kernel = product_kernel.k2
                if hasattr(rbf_kernel, 'length_scale'):
                    length_scales = rbf_kernel.length_scale
                    print(f"RBF Kernel:")
                    if len(length_scales) >= 2:
                        print(f"  O2 퍼센트 Length Scale: {length_scales[0]:.6f}")
                        print(f"  두께 Length Scale: {length_scales[1]:.6f}")
                    else:
                        print(f"  Length Scale: {length_scales:.6f}")
                    ...
                    if hasattr(rbf_kernel, 'length_scale_bounds'):
                        ...
                        if bounds != 'fixed':
                            if len(bounds) >= 2:
                                print(f"  O2 Length Scale Bounds: ({bounds[0][0]:.2f}, {bounds[0][1]:.2f})")
                                print(f"  두께 Length Scale Bounds: ({bounds[1][0]:.2f}, {bounds[1][1]:.2f})")
                            else:
                                print(f"  Length Scale Bounds: {bounds}")
                        else:
                            print(f"  Length Scale Bounds: fixed")

        # Sum의 두 번째 부분: WhiteKernel
        if hasattr(composite_kernel, 'k2'):
            white_kernel = composite_kernel.k2
            if hasattr(white_kernel, 'noise_level'):
                noise_level = white_kernel.noise_level
                print(f"WhiteKernel:")
                print(f"  Noise Level: {noise_level:.8f}")

                if hasattr(white_kernel, 'noise_level_bounds'):
                    noise_bounds = white_kernel.noise_level_bounds
                    if noise_bounds != 'fixed':
                        print(f"  Noise Level Bounds: ({noise_bounds[0]:.2e}, {noise_bounds[1]:.2e})")
                    else:
                        print(f"  Noise Level Bounds: fixed")

    except Exception as e:
        print(f"복합 커널 파라미터 추출 중 오류: {e}")
        print("대안 방법으로 파라미터를 추출합니다...")

        # 대안 방법: theta 파라미터 직접 접근
        try:
            if hasattr(gpr.kernel_, 'theta'):
                theta = gpr.kernel_.theta
                print(f"커널 파라미터 벡터 (theta): {theta}")

                # 일반적인 ConstantKernel*RBF+WhiteKernel 구조에서 theta 순서:
                # [log(constant), log(length_scale_1), log(length_scale_2), log(noise_level)]
                if len(theta) >= 4:
                    print(f"파라미터 해석:")
                    print(f"  Log Constant Value: {theta[0]:.6f} → Constant: {np.exp(theta[0]):.6f}")
                    print(f"  Log O2 Length Scale: {theta[1]:.6f} → Length Scale: {np.exp(theta[1]):.6f}")
                    print(f"  Log 두께 Length Scale: {theta[2]:.6f} → Length Scale: {np.exp(theta[2]):.6f}")
                    print(f"  Log Noise Level: {theta[3]:.6f} → Noise Level: {np.exp(theta[3]):.8f}")
                elif len(theta) >= 3:
                    print(f"파라미터 해석 (3개):")
                    print(f"  Log Constant/Length Scale 1: {theta[0]:.6f} → Value: {np.exp(theta[0]):.6f}")
                    print(f"  Log Length Scale 2: {theta[1]:.6f} → Value: {np.exp(theta[1]):.6f}")
                    print(f"  Log Noise Level: {theta[2]:.6f} → Value: {np.exp(theta[2]):.8f}")
                else:
                    for i, param in enumerate(theta):
                        print(f"  Theta[{i}]: {param:.6f} → Exp: {np.exp(param):.6f}")

        except Exception as e2:
            print(f"대안 방법도 실패: {e2}")

    # 3. GPR 모델 파라미터 (기존과 동일)
    print(f"\nGPR 모델 파라미터:")
    ...
    try:
        ...
        print(f"  Log Marginal Likelihood: {gpr.log_marginal_likelihood():.8f}")
    except Exception as e:
        print(f"모델 통계 추출 중 오류: {e}")
...
while True:
    ...
    kernel = ConstantKernel(
        constant_value=1.0,
        constant_value_bounds=(0.1, 1000.0)
    ) * RBF(
        length_scale=[1.0, 15.0],  # O2 퍼센트(0-16%), 두께(10-90nm)에 맞게 조정
        length_scale_bounds=[(0.5, 10.0), (1.6, 100.0)]  # nm 범위에 맞는 경계값
    ) + WhiteKernel(
        noise_level=0.001,
    ...
    print_hyperparameters_composite(gpr, iteration)
