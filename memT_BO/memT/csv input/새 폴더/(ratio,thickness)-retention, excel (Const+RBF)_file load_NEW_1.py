...
def print_hyperparameters(gpr, iteration):
    ...
    """
    print(f"\n=== 하이퍼파라미터 확인 (반복 {iteration + 1}) ===")

    # 1. 기본 GPR 파라미터
    print(f"GPR alpha 값: {gpr.alpha}")
    print(f"normalize_y: {gpr.normalize_y}")
    print(f"n_restarts_optimizer: {gpr.n_restarts_optimizer}")

    # 2. 최적화된 커널 파라미터
    optimized_kernel = gpr.kernel_
    print(f"\n최적화된 커널 구조:")
    print(f"  전체 커널: {optimized_kernel}")

    # 3. ConstantKernel 파라미터
    constant_value = optimized_kernel.k1.k1.constant_value
    print(f"\nConstantKernel 파라미터:")
    print(f"  constant_value: {constant_value:.6f}")

    # 4. RBF 커널 파라미터 (length_scale)
    length_scales = optimized_kernel.k1.k2.length_scale
    print(f"\nRBF Kernel 파라미터:")
    print(f"  length_scale (O2 퍼센트): {length_scales[0]:.6f}")
    print(f"  length_scale (두께): {length_scales[1]:.6f}")

    # 5. WhiteKernel 파라미터 (노이즈)
    ...
    print(f"\n모델 성능 지표:")
    print(f"  Log Marginal Likelihood: {log_likelihood:.6f}")

    print("=" * 50)

# 1. 실험 가능한 O2/(Ar+O2) 퍼센트 비율 후보 생성 (0%부터 32%까지, 4%씩 증가)
o2_values = np.array([0, 2, 4, 6, 8, 10, 12, 14, 16]) # 9가지 O2 비율
...
thickness_nm = np.array([10, 20, 30, 40, 50, 60, 70, 80, 90]) # 9가지 두께 (nm 단위)
...
# 5. 베이지안 최적화 루프 (*** 하이퍼파라미터 확인 기능 추가 ***)
iteration = 0
...
while True:
    ...
    kernel = ConstantKernel(
        ...
        length_scale=[10.0, 25.0], # O2 퍼센트(0-32%), 두께(10-90nm)에 맞게 조정
        length_scale_bounds=[(0.5, 80.0), (2.0, 160.0)] # nm 범위에 맞는 경계값
    ) + WhiteKernel(
        noise_level=0.001,
