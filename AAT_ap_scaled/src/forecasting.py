...
def run_rbf_sigma_sweep(
    ...
    if center_mode == "distinct":
        ...
        distance_reference = median_nearest_center_distance(standardized.x_train, centers_fixed)
    ...
    for n_sigmas in n_values:
        ...
        if center_mode == "replicated":
            temp_centers, _ = compute_replicated_centers_and_sigmas(
                standardized.x_train,
                n_clusters=n_sigmas,
                c_total=c_total,
                sigma_set=sigma_set_exp,
                seed=seed,
            )
            distance_reference = median_nearest_center_distance(standardized.x_train, temp_centers)
        assert distance_reference is not None
...
def run_forecasting_experiment(
    ...
    distance_reference = sweep["sigma_distance_reference"]
