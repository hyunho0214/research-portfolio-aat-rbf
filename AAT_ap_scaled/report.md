...
`AAT_ap_2`에는 비교 목적으로 `--center-mode replicated`도 추가했습니다. 이 모드는 RBF2/Figure 4 흐름처럼 `KMeans(n_clusters=N)`을 수행한 뒤 center와 sigma를 `C_total`까지 복제합니다. 이 경우 `N`은 sigma diversity뿐 아니라 unique center 수와 유효 모델 용량도 함께 바꾸므로, Figure 재현에는 유리하지만 순수 sigma-diversity ablation으로 해석할 때는 주의가 필요합니다.
...
비교 재현용으로 `--sigma-mode direct`도 제공합니다. 이 모드는 RBF2처럼 experimental sigma 값을 model sigma로 직접 사용합니다.
